...
<img width="920" height="520" alt="image" src="https://github.com/user-attachments/assets/06dd04ba-f0c5-4e82-a71b-e77ba6a24298" />

Seletores de dados ajudam a refinar os eventos gerados nos manipuladores de eventos. Você pode especificar vários critérios no seletor de dados, incluindo dispositivos, sub-redes e filtros. É necessário configurar um seletor de dados antes de aplicá-lo a um manipulador de eventos. 

Os filtros são regras granulares que filtram quais tipos de logs correspondem ao seletor de dados. Você pode criar vários filtros por seletor de dados. O seletor de dados compara os filtros usando a lógica OR.

A imagem mostra os campos que você pode configurar para um filtro, incluindo o tipo de dispositivo de log, o tipo de log e subtipo de log. Observe que os filtros suportam lógica AND e OR. Ao configurar um filtro, pode-se selecionar critérios comuns na interface gráfica do usuário ou usar filtros de texto genéricos, caso precise de uma filtragem precisa. 

<img width="733" height="414" alt="image" src="https://github.com/user-attachments/assets/ce78b867-9f0a-42ae-8434-348c4c857a4c" />

Eventos no FortiAnalyzer podem ter um de quatro status. O estado atual do evento determina se o time de segurança precisa tomar alguma providência ou não. 

Os status possíveis são:

* Não tratado (unhandled): O evento de risco de segurança não é mitigado ou contido, então é considerado aberto. Por exemplo, um log de IPS/AV com action=pass irá ter o status de evento "unhandled". Um evento de Botnet e IoC também é considerado "unhandled".
* Contido (Contained): A origem do risco é isolado. Por exemplo, um log de antivirus com ação de quarentena terá o status de evento Contained.
* Mitigado (Mitigated): O risco de segurança é mitigado por meio de bloqueio ou descarte. Por exemplo, um sistema de prevenção de intrusão (IPS) ou log de antivirus com ação de bloqueio ou descarte terá o status de evento "mitigated".
* Em branco - (Blank): O status blank indica que o risco está em um estado diferente de não tratado, contido ou mitigado. Por exemplo, ações de permissão e bloqueio podem ser vistas nos registros associados ao evento.

<img width="661" height="374" alt="image" src="https://github.com/user-attachments/assets/72669844-b36b-48af-a649-a4e45e5b820a" />
